...
static uint8_t exp_data[10] ={0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
		...
		switch (res){
          ...
          case MSM_SENSOR_RES_HDR:
           ...
           case MSM_SENSOR_RES_LLS:
              ...
              cmd_data[0] = exp_data[0];
              cmd_data[1] = exp_data[1];
              cmd_data[2] = exp_data[2];
              cmd_data[3] = exp_data[3];
              cmd_data[4] = 0x00;
              cmd_data[5] = 0x00;
              cmd_data[6] = exp_data[0];
              cmd_data[7] = exp_data[1];
              cmd_data[8] = exp_data[2];
              cmd_data[9] = exp_data[3];
              cmd_data[10] = 0x00;
              cmd_data[11] = 0x00;

              ce1702_i2c_write(ce1702_s_interface_ctrl->sensor_i2c_addr, 0x19, cmd_data, 12);
              LDBGI(">>>>>>>>>>>> TV value under for preview = 0x%x\n", cmd_data[0]);
              LDBGI(">>>>>>>>>>>> TV value upper for preview = 0x%x\n", cmd_data[1]);
              LDBGI(">>>>>>>>>>>> SV value under for preview = 0x%x\n", cmd_data[2]);
              LDBGI(">>>>>>>>>>>> SV value upper for preview = 0x%x\n", cmd_data[3]);
              LDBGI(">>>>>>>>>>>> TV value under for capture = 0x%x\n", cmd_data[6]);
              LDBGI(">>>>>>>>>>>> TV value upper for capture = 0x%x\n", cmd_data[7]);
              LDBGI(">>>>>>>>>>>> SV value under for capture = 0x%x\n", cmd_data[8]);
              LDBGI(">>>>>>>>>>>> SV value upper for capture = 0x%x\n", cmd_data[9]);
	...
	switch (ce1702_frame_mode) {
		case CE1702_MODE_PREVIEW :
			if (isPreviewMode == 1){
              ...
              rc = ce1702_i2c_read(ce1702_s_interface_ctrl->sensor_i2c_addr, 0x13, cmd_data, 4, exp_data,10);
              if (rc < 0)
					LDBGE("%s: ce1702_i2c_read failed for 0x13 \n", __func__);

              LDBGI(">>>>>>>>>>>> TV value under = 0x%x\n", exp_data[0]);
              LDBGI(">>>>>>>>>>>> TV value upper = 0x%x\n", exp_data[1]);
              LDBGI(">>>>>>>>>>>> SV value under = 0x%x\n", exp_data[2]);
              LDBGI(">>>>>>>>>>>> SV value upper= 0x%x\n", exp_data[3]);
              LDBGI(">>>>>>>>>>>> AV value under = 0x%x\n", exp_data[4]);
              LDBGI(">>>>>>>>>>>> AV value upper = 0x%x\n", exp_data[5]);
              LDBGI(">>>>>>>>>>>> BV value under = 0x%x\n", exp_data[6]);
              LDBGI(">>>>>>>>>>>> BV value upper = 0x%x\n", exp_data[7]);
              LDBGI(">>>>>>>>>>>> BV value under of taken image = 0x%x\n", exp_data[8]);
              LDBGI(">>>>>>>>>>>> BV value upper of taken image = 0x%x\n", exp_data[9]);
